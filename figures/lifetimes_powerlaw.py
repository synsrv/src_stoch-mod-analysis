    ...
    for npool in all_Npool:
        ...
        for df in df_all:
            ...
            # if df['Npool']==npool and df['k'] in [1000]:
                ...
                lts = NN*-1.*np.diff(df['synsrv_prb'])
                lts = lts.astype(int)
                
                ts = dt*np.array(range(1,len(df['synsrv_prb'])))
                
                lts_dat = np.repeat(ts, lts)
                print(lts_dat)
        ...
        pl.legend()
        pl.savefig('figures/lifetimes_powerlaw/xx2x.png',
                            bbox_inches='tight')
