...
def k_effect_figures(fit=False):
    ...
    for dpath in data_dirs[1:]:
        ...
        try:
            ...
            with open(dpath+'/synsrv_prb_start.p', 'rb') as pfile:
    ...
    all_start = np.unique([df['start'] for df in df_all])

    for npool, start in itertools.product(all_Npool,all_start):
        ...
        for df in df_all:
            ...
            if df['Npool']==npool and df['start']==start:
        ...
        directory = 'figures/meta_k_effect/start_%d/' % start
